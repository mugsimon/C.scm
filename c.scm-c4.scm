...
(define (c4hoist-let-fun def)
  (let ((var (car def))
        (form (cdr def)))
    (set! c.scm:*c4local-functions* (cons (list var (c4lam form)) c.scm:*c4local-functions*))))
    ...
    (cond ((null? defs)
          ...
          (else
           (let ((var (caar defs))
                 ...
                 (form0 (caddar defs)))
             (if (var-local-fun var)
                 (begin (c4hoist-letrec-fun (car defs))
                        (loop (cdr defs)
                              cdefs))
                 (loop (cdr defs)
                       (cons (list var
